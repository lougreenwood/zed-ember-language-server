...
const SERVER_PATH: &str = "node_modules/@ember-tooling/ember-language-server/bin/server.js";
const PACKAGE_NAME: &str = "@ember-tooling/ember-language-server";
...
impl zed::Extension for EmberExtension {
    ...
    fn language_server_command(
        ...
        let ts_lib_path = node_modules_path
            ...
            .to_string_lossy()
            .to_string();
        let ember_service_path = node_modules_path
            .join("@ember-tooling/ember-language-server/bin")
            .to_string_lossy()
            .to_string();
        println!(
            "command: {:?} --stdio",
            full_path_to_server, ts_lib_path, ember_service_path
        );
